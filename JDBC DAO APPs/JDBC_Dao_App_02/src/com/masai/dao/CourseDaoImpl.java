...
public class CourseDaoImpl implements CourseDao {
	...
	public List<StudentCourseDTO> getAllStudentByCourseName(String cname) throws CourseException {
		...
		try(Connection conn = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = conn.prepareStatement("select s.roll, s.name, s.email, s.address, c.cname, c.duration from student s INNER JOIN course c INNER JOIN student_course sc ON s.roll = sc.roll AND c.cid = sc.cid AND c.cname = ?");
